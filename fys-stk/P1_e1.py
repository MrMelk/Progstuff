...
for i in range(len(params)):
    CIlow.append(params[i] - 1.96*np.std(x)/np.sqrt(len(x)))
    CIhigh.append(params[i] - 1.96*np.std(x)/np.sqrt(len(x)))
...
ymean = 1/n *np.sum(y)
R2 = 1- (np.sum((y - pred)**2))/(np.sum(y - ymean)**2)


xylist = []
for i in range(len(y)):
    xylist.append((x[i], y[i]))
X_train, X_test, y_train, y_test = train_test_split(xylist, z, test_size=0.33, random_state=42)

#train_test = train_test_split(x,y,z)
#train = train_test[0]
